...
from spr_pick.models.feature_extractor import ResNet, ResNet6, ResNet8, ResNet8_mod
...
class DualNetwork(nn.Module):
    def __init__(
        ...
        if self.detect:
            ...
            # self.detect_block =LinearClassifier(ResNet8_mod(bn=True))
            # print('self.detect_block')
            # print(self.detect_block)
            self.output_conv_f = self.Conv2d(96, 1, 1)
    ...
    def forward(self, x: Tensor):
        ...
        if self.blindspot:
            ...
            if self.detect:
                ...
                place_holder = None
                # x_blind = self.bn(x_blind)
                # x_detect = self.detect_block(x_blind)
                # print('x_detect', x_detect.shape)
...
def make_saliency_layer(curr_dim):
    return nn.Sequential(convolution(7, curr_dim, curr_dim*8, with_bn=False),
        # nn.Upsample(scale_factor=2, mode="nearest"),
...
class JointNetwork(nn.Module):
    def __init__(
        ...
        self.denoise_branch = DualNetwork(
