...
from spr_pick.utils.utils import insize_from_outsize
...
class ResNet(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.pad: ## add (width-1)//2 zeros to edges of x
            ...
            x = F.pad(x, (p,p,p,p))
        # print('x', x.shape)
        # print('filled features', self.features)
        z = self.features(x)
        # print('z', z.shape)
        return z
...
class ResNet6(ResNet):
    def make_modules(self, units=[32, 64, 128], bn=True, dropout=0.0, activation=nn.ReLU, **kwargs):
        ...
        modules.append(MaxPool(3, stride=1))
        # modules.append(MaxPool(4, stride=2))
        if dropout > 0:
...
class ResNet8_mod(ResNet):
    def make_modules(self, units=[32, 64, 128], bn=True, dropout=0.0
        ...
        modules = [
                BasicConv2d(96, units[0], 7, stride=stride, bn=bn, activation=activation),
                ]
        if pooling is not None:
            modules.append(pooling(3, stride=1))
        if dropout > 0:
