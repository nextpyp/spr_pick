...
class NoiseEstNetwork(nn.Module):
	def __init__(
		...
		super(NoiseEstNetwork, self).__init__()
		...
		self.encode_block_1 = nn.Sequential(
			self.Conv2d(in_channels, 16, 3, stride=1, padding=1),
			nn.LeakyReLU(negative_slope=0.1, inplace=True),
			# nn.InstanceNorm2d(48),
			self.Conv2d(16, 16, 3, padding=1),
			nn.LeakyReLU(negative_slope=0.1, inplace=True),
			# nn.InstanceNorm2d(48),
		...
		def _encode_block_2_3_4_5() -> nn.Module:
			return nn.Sequential(
				self.Conv2d(16, 16, 3, stride=1, padding=1),
				nn.LeakyReLU(negative_slope=0.1, inplace=True),
				# nn.InstanceNorm2d(48),
				_max_pool_block(nn.MaxPool2d(2)),
			)
		...
		self.decode_block_5 = nn.Sequential(
			self.Conv2d(32, 32, 3, stride=1, padding=1),
			nn.LeakyReLU(negative_slope=0.1, inplace=True),
		...
		def _decode_block_4_3_2() -> nn.Module:
			return nn.Sequential(
				self.Conv2d(48, 32, 3, stride=1, padding=1),
				nn.LeakyReLU(negative_slope=0.1, inplace=True),
				# nn.InstanceNorm2d(96),
				self.Conv2d(32, 32, 3, stride=1, padding=1),
				nn.LeakyReLU(negative_slope=0.1, inplace=True),
		...
		self.decode_block_1 = nn.Sequential(
			self.Conv2d(32 + in_channels, 32, 3, stride=1, padding=1),
			nn.LeakyReLU(negative_slope=0.1, inplace=True),
			# nn.InstanceNorm2d(96),
			self.Conv2d(32, 32, 3, stride=1, padding=1),
			nn.LeakyReLU(negative_slope=0.1, inplace=True),
