...
class NoisyDataset(Dataset):
    ...
    def prepare_input(
        ...
        #noisy_in, noisy_in_coeff = joint.utils.noise.add_style(clean, self.noise_style)
        noisy_in, noisy_in_coeff = clean, 0
        # print('noisy_in')
        # print(noisy_in.shape)
        if self.algorithm == NoiseAlgorithm.NOISE_TO_VOID and self.training_mode:
            noisy_in, mask_coords = spr_pick.utils.n2v_ups.manipulate(noisy_in, 5) # TODO use config for neighbourhood radius
        ...
        # N2N and N2V require noisy input and noisy reference images
        elif (
                self.algorithm == NoiseAlgorithm.NOISE_TO_NOISE or
                self.algorithm == NoiseAlgorithm.NOISE_TO_VOID
            ):
            ref, ref_coeff = spr_pick.utils.noise.add_style(clean, self.noise_style)
